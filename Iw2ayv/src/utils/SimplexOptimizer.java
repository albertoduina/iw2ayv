...
public class SimplexOptimizer {
	...
	Hashtable<String, Class<?>> mClassMap;
	...
	public SimplexOptimizer() {
		...
		mRho = 1.0D; // origin. 1.0D
		mChi = 1D; // origin. 1D
		mGamma = 0.5D; // origin. 0.5D;
		mSigma = 0.5D; // origin. 0.5D;
		mMaxIterations = 10000;
		mMaxRestarts = 2; // origin 2
		mTolerance = 0.00000001D; // 0.00000001D;
		mClassMap = new Hashtable<String, Class<?>>();
	...
	public void setObjectAndMethod(Object obj, String s, int i, double d)
			throws Exception {
		Method amethod[] = obj.getClass().getMethods();
		boolean flag = true;
		for (int j = 0; j < amethod.length; j++)
			if (amethod[j].getName().equals(s)
					&& amethod[j].getParameterTypes().length == i)
				flag = false;

		if (flag) {
			throw new Exception("Could not find a method named: " + s
	...
	public void initialize() throws Exception {
		...
		for (int j = 0; j < mAllParameters.length; j++) {
			...
			if (mClassMap.get(aclass[j].getName()) != null)
				aclass[j] = (Class<? extends Object>) mClassMap.get(aclass[j].getName());
		}
		...
		for (int l = 0; l < vector.size(); l++)
			mOrigGuesses.setParameter(l, ((Double) vector.elementAt(l))
					.doubleValue());
	...
	/*
	 * private String dumpVertex(Vertex vertex) { String s = "VERTEX: "; s = s +
	 * "\nR[" + vertex.getResidual() + "]\t"; for(int i = 0; i <
	 * vertex.getNumParams(); i++) s = s + "P" + i + "[" +
	 * vertex.getParameter(i) + "]\t";
	 * 
	...
	Random random = new Random(10000000); // DEBUG - to make artifacts easier
	...
	private Vertex[] generateSimplex(Vertex vertex) {
		...
		for (int i = 0; i < vertex.getNumParams(); i++) {
			...
			for (int j = 0; j < vertex1.getNumParams(); j++)
				vertex1.setParameter(j, random.nextDouble()
						* vertex.getParameter(j) + random.nextDouble()
						* mOrigGuesses.getParameter(j));
	...
	private double calculateResidual(Vertex vertex) throws Exception {
		...
		double d = ((Double) mMethod.invoke(mObject, mAllParameters))
				.doubleValue();
	...
	private void iterate() throws Exception {
		...
		if (vertex1.getResidual() <= mSimplex[0].getResidual()) {
			...
			if (vertex2.getResidual() < vertex1.getResidual()) {
				...
				if (DEBUG)
					ij.IJ.log("REFLECT-EXPAND");
			} else {
				if (DEBUG)
					System.out.println("REFLECT ONLY");
				if (DEBUG)
					ij.IJ.log("REFLECT ONLY");
		...
		// 3- reflected point is not the new best
			...
			if (vertex1.getResidual() >= mSimplex[mSimplex.length - 2]
					.getResidual()) {
				...
				// and return (D)
				if (vertex3.getResidual() >= mSimplex[mSimplex.length - 2]
						.getResidual()) {
	...
	private Vertex reflect(Vertex vertex, Vertex vertex1, double d)
			throws Exception {
	 ...
	 */
	private Vertex expand(Vertex vertex, Vertex vertex1, double d)
			throws Exception {
		Vertex vertex2 = new Vertex(vertex1.getNumParams());
		for (int i = 0; i < vertex1.getNumParams(); i++) {
			double d1 = vertex.getParameter(i);
	 ...
	 */
	private Vertex outsideContract(Vertex vertex, Vertex vertex1, double d)
			throws Exception {
		Vertex vertex2 = new Vertex(vertex1.getNumParams());
		for (int i = 0; i < vertex1.getNumParams(); i++) {
			double d1 = vertex.getParameter(i);
	...
	private Vertex[] shrink(Vertex avertex[], double d) throws Exception {
		for (int i = 1; i < avertex.length; i++) {
			for (int j = 0; j < avertex[0].getNumParams(); j++) {
				double d1 = avertex[i].getParameter(j)
						+ d
						* (avertex[0].getParameter(j) - avertex[i]
								.getParameter(j));
	...
	private Vertex[] order(Vertex avertex[]) {
		...
		mLastTolerance = Math
				.abs((avertex[avertex.length - 1].getResidual() - avertex[0]
						.getResidual())
						/ avertex[0].getResidual());

		return avertex;
	}

	/*
	 * private static String dumpObjectArray(Object aobj[]) { String s = "[";
	 * for(int i = 0; i < aobj.length; i++) s = s + ", " + aobj[i];
	 * 
	 * s = s + "]"; return s; }
	 */
	...
	private void checkMethodIsDefined() throws Exception {
		if (mAllParameters == null)
			throw new Exception(
					"You must call setObjectAndMethod(...) prior to setting paramters");
		else
			return;
	}
