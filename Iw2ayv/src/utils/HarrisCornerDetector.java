...
public class HarrisCornerDetector {
	...
	/*
	 * Default parameters; a (usually modified) instance of this class may be
	 * passed to constructor of the main class.
	 */
	...
	private final float[] bfilt = { 0.01563f, 0.09375f, 0.234375f, 0.3125f,
	...
	public PointRoi returnCorners() {
		int i1=0;
		float[] vetx = new float[corners.size()];
		float[] vety = new float[corners.size()];
		for (Corner ipt : corners) {
			vetx[i1] = ipt.u;
			vety[i1]= ipt.v;
			i1++;
		}
		PointRoi pr1 = new PointRoi(vetx, vety);
		return pr1;
	}
	...
	private boolean isLocalMax(FloatProcessor fp, int u, int v) {
		...
		if (u <= 0 || u >= w - 1 || v <= 0 || v >= h - 1)
			...
			float cp = pix[i1];
			return cp >= pix[i0 - 1] && cp >= pix[i0] && cp >= pix[i0 + 1]
	...
	public ImageProcessor showCornerPoints(ImageProcessor ip, int nmax) {
		ByteProcessor ipResult = (ip instanceof ByteProcessor) ? (ByteProcessor) ip
				.duplicate() : (ByteProcessor) ip.convertToByte(true);
